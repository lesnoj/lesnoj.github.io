...
*/
            tooltipTextDraganddelete: 'Можно <b>переместить точку </b>мышью<br><b>Убрать точку </b>щелчком мыши, удерживая SHIFT',
            tooltipTextResume: '<br><b>Продолжить линию </b>щелчком мыши, удерживая CTRL',
            tooltipTextAdd: '<b>Добавить точку </b>щелчком мыши, удерживая CTRL',
             ...
             * @default
             */
            measureControlLabel: '<img class="icon" src = "rul/trans.svg">',
             /**
             ...
             */
            unitControlTitle: {
               text: 'Поменять единицы измерения',
               metres: 'метры',
               landmiles: 'сухопутные мили',
...
*/
            ...
            fixedLine: {
                 ...
                 * @default
                 */
                weight: 3 //тощина окончательной полилинии
            },
            /**
        ...
        onAdd: function(map) {
            ...
            if (this.options.showUnitControl) {
			    ...
			    //Добавляю кнопку сохранения
	        var label = '<a download="drawnlines.json"/><img style="pointer-events: none;" class="icon" src="track/search.svg"/></a>'
	        this._saveControl = this._createControl (label, title, classes,  this._container, this._save, this);
            this._saveControl.title = 'Сохранить линии (GeoJSON)';
		...
		//добавил функцию сохранения
	_save: function(ev) { 

			//Заготовка атрибутов для geoJSON
        var geojson = {};
        geojson['type'] = 'Feature';
        geojson['geometry'] = {};
        geojson['geometry']['type'] = "MultiLineString";
        
        coordinates = [];  // пустой массив координат в пределах одной полилинии
		sumcoordinates = [];  // пустой массив координат разных полилиний

        for (var i = 0; i < this._arrPolylines.length; i++) {  // по числу законченных полилиний
			
			for (var j = 0; j < this._arrPolylines[i].circleCoords.length; j++) {  // по числу точек в каждой полилинии
			
				coordinates.push([this._arrPolylines[i].circleCoords[j].lng, this._arrPolylines[i].circleCoords[j].lat]) // в массив coordinates добавляются точки одной полилинии
			}
			
			sumcoordinates.push(coordinates); // в массив sumcoordinates добовляются законченные полилинии
			coordinates = []; // очистка точек полилинии, уже добавленной в массив sumcoordinates, чтобы освободить массив coordinates для точек следующей полилинии
			
        }

		geojson['geometry']['coordinates'] = sumcoordinates;  // массив, содержащий данные о всех точках всех полилиний, добавляется в строку
		
      //tmp = {"type": "FeatureCollection","features": [{"type":"Feature","geometry": tmp }]}
      if ('msSaveOrOpenBlob' in navigator) navigator.msSaveOrOpenBlob(new Blob([JSON.stringify(geojson)]), "drawnlines.json"); // для поддержки старых браузеров
      else ev.target.href = "data:application/geo+json," + JSON.stringify(geojson)  // преобразуется в geoJSON и добавляется в сторку HTML, откуда прило событие
        ...
        _onKeyDown: function (e) {
            if (e.keyCode === 27) {
              ...
              //    text = text + '<div class="polyline-measure-tooltip-difference">+' + '0</div>';  // Показывает разницу расстояний между двумя последними точками
     ...
     //         textCurrent += '<div class="polyline-measure-tooltip-difference">+' + differenceRound.value + '&nbsp;' +  differenceRound.unit + '</div>';  // Показывает разницу расстояний между двумя последними точками
            }
		...
		//Рисует всплывающую подсказку к точкам
