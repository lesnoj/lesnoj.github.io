...
//Начало маркеров-картинок со всплывающими окнами
        ...
        for (var i = 0; i < IconPhoto.length; i++){
			...
			var marker = new L.Marker([IconPhoto[i][0],IconPhoto[i][1]], {icon:IconImg, title:IconPhoto[i][2], riseOnHover:true}).bindPopup(L.popup({minWidth:660, maxWidth:660, maxHeight:500}).setContent(IconPhoto[i][3])).addTo(map);
		}
...
//Начало блока отображения загружаемого трека GPX и ему подобных
        ...
        var style = {
          ...
          opacity: 1.0,
          fillOpacity: 1.0,
          weight: 5, // тощина отображемого загруженного трека (для компа - 3, для мобилы - 5)
          clickable: false
        };
