...
(function (factory, window) {
    ...
    var FileLoader = L.Layer.extend({
        ...
        _loadGeoJSON: function _loadGeoJSON(content) {
            ...
            layer10 = this.options.layer(content, this.options.layerOptions);

            if (layer10.getLayers().length === 0) {
                throw new Error('GeoJSON has no valid layers.');
            }

            if (this.options.addToMap) {
 
      		//Забросил данные из полученного фйла на слой WayGroup и добавил слой на карту  
   		WayGroup.addLayer(layer10);
   		this._map.addLayer(WayGroup);

		/*отключил загузку данных напрямую на карту
	 	layer10.addTo(this._map);  // загрузка на карту слоя с данными из полученного фйла
		*/
            }
            return layer10;
        },
    ...
    var FileLayerLoad = L.Control.extend({
        statics: {
            TITLE: 'Открыть файл (GPX, KML, GeoJSON)',
            LABEL: '&#8965;'
        },
        options: {
            position: 'topleft',
        ...
        _initContainer: function () {
	        ...
	        link.classList.add('polyline-measure-clearControl1');  //Добавил вспыхнивание кнопки зеленым при нажатии из css линейки (rul/Leaflet.PolylineMeasure). В css было -  a.polyline-measure-clearControl:active {background-color: #8f8;};
		...
		//Кнопка очистки
            var link1 = L.DomUtil.create('a', 'leaflet-bar-part', container);
            link1.innerHTML = '<b><font size="4">&times;</font></b>';
            link1.href = '#';
            link1.title = 'Очистить импортированные данные';
	        link1.classList.add('polyline-measure-clearControl');  //Добавил вспыхнивание кнопки при нажатии из css линейки (rul/Leaflet.PolylineMeasure). В css было -  a.polyline-measure-clearControl:active {background-color: #f88;}
	        L.DomEvent.disableClickPropagation(link1);
	       // Очистка слоя WayGroup по клику на кнопке
	        L.DomEvent.on(link1, 'click', function (e) {
		      WayGroup.clearLayers();
      		  e.preventDefault();
    		});
            return container;
        }
    ...
    //добавил глобальный слой для заброски данных из файла
    WayGroup = new L.FeatureGroup();
